...
const populationNumbers = Array.from(populationSpans).map((span)=>parseInt(span.textContent.replace(/,/g, ""))).filter((value)=>!isNaN(value));
...
if (averagePopulationSpan) averagePopulationSpan.textContent = formatNumber(Math.round(averagePopulation));
if (totalPopulationSpan) totalPopulationSpan.textContent = formatNumber(totalPopulation);
